...
def build_model(model):
    ...
    inpoints = tf.placeholder(dtype=tf.float32, shape=[batch_size, 5000, 2])
    gt3d = tf.placeholder(dtype=tf.float32, shape=[batch_size, cfg.gt_size, 3])
...
def compute_predictions():
    ...
    checkpoint_file = os.path.join(exp_dir, 'model-{}'.format(cfg.cc_n))
    ...
    save_pred_dir = os.path.join(exp_dir, cfg.save_predictions_dir + '_' + cfg.other_class)
        ...
        for view_idx in range(num_views):
            ...
            if save_pred:
                ...
                if mm != 2048:
                    a = int(2048 / mm)
                    b = int(2048 % mm)
                    pc_np = np.tile(pc_np, [1, a, 1])
                    cc = pc_np[:, :b, :]
                
                    pc_np = np.concatenate([pc_np, cc], 1)
                all_pcs[view_idx, :, :] = np.squeeze(pc_np)
                all_z_latent[view_idx] = np.zeros([1024]) #:z_latent_np
                if cfg.predict_pose:
