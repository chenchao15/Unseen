...
def build_model(model):
    ...
    masks_sdf = tf.placeholder(dtype=tf.float32, shape=[batch_size,cfg.image_size,cfg.image_size,1])
...
def compute_predictions():
    ...
    checkpoint_file = os.path.join(exp_dir, 'model-{}'.format(cfg.test_model))
    print("restoring checkpoint", checkpoint_file)
    restorer.restore(sess, checkpoint_file)

    save_dir = os.path.join(exp_dir, '{}_best_proj'.format(cfg.save_predictions_dir))
    mkdir_if_missing(save_dir)
    save_pred_dir = os.path.join(exp_dir, cfg.save_predictions_dir + '_best')
    mkdir_if_missing(save_pred_dir)
    save_coord_dir = os.path.join(exp_dir, '{}_best_coord'.format(cfg.save_predictions_dir))
    mkdir_if_missing(save_coord_dir)
    ...
    for k in range(num_models):
        ...
        if save_pred:
            ...
            all_coords = np.zeros((num_views, pc_num_points, 2))
        ...
        for view_idx in range(num_views):
            ...
            if cfg.pc_rgb:
                ...
                proj_tensor = projs
            (coords, pc_np, rgb_np, proj_np, cam_transf_np, z_latent_np) = sess.run([coord, point_cloud, rgb, proj_tensor, cam_transform, z_latent],
                                                               feed_dict={input_image: input_image_np,
                                                                          cam_matrix: extr_mtr,
                                                                          cam_quaternion: cam_quaternion_np})
      
            np.save('{}/{}_coord.npy'.format(save_coord_dir, sample.name), coords)            
            ...
            all_coords[view_idx, :, :] = np.squeeze(coords) 
        ...
        if save_pred:
            np.save('{}/{}_masks.npy'.format(save_coord_dir, sample.name), masks)
            np.save('{}/{}_coord.npy'.format(save_coord_dir, sample.name), all_coords)
            if cfg.save_as_mat:
