...
def run_eval():
    ...
    for k in range(num_models):
        ...
        maxxyz = np.max(Vgt, 0)
        lines = (maxxyz - minxyz)
        djiaoxian = math.sqrt(lines[0]*lines[0] + lines[1]*lines[1] + lines[2]*lines[2])
        
        print(np.mean(Vgt, axis=0))
        ...
        for i in range(num_views):
            ...
            #if sample.name == '7a2d21eddb89c8242058afcc28d23393':
                #np.save('cc_'+str(i)+'.npy', pred)
                #np.save('gt_'+str(i)+'.npy',Vgt)
            minxyz = np.min(pred, 0)
            maxxyz = np.max(pred, 0)
            lines = (maxxyz - minxyz)
            djiaoxian = math.sqrt(lines[0]*lines[0] + lines[1]*lines[1] + lines[2]*lines[2])
            pred = pred / djiaoxian
            minxyz = np.min(pred, 0)
    ...
    scipy.io.savemat(os.path.join(exp_dir, "2os_chamfer_{}.mat".format(save_pred_name)),
                     {"chamfer": chamfer_dists,
                      "model_names": to_np_object(model_names)})

    file = open(os.path.join(exp_dir, "2os_chamfer_{}.txt".format(save_pred_name)), "w")
