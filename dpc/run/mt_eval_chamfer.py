...
def get_data():
    ...
    for k in range(num_models):
        ...
        Vgt = obj["points"]
        preds.append(all_pcs)
        gts.append(Vgt)
    return model_names, preds, gts
...
def run_eval(datas):
    ...
    for k in range(num_models - 1):
        ...
        chamfer_dists = np.concatenate((chamfer_dists2, np.expand_dims(current_mean, 0)))

    final = np.mean(chamfer_dists, axis=0) * 100
    print(final)
