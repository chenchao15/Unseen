...
def tf_records_dataset_filename(cfg):
    '''if cfg.test_other_class:
        cfg.num_views = 20
        filename = '{}_testmodel_all_gt_8000_patch.tfrecords_2048'.format(cfg.other_class)
    else:
        filename = '{}_testmodel_all_gt_8000_patch.tfrecords'.format(cfg.synth_set)'''
    filename = '{}_testmodel_all_gt_8000_patch_{}.tfrecords'.format(cfg.other_class, cfg.test_point_num)
    path = os.path.join(cfg.inp_dir, filename)
    if not os.path.exists(path):
        filename = '{}_testmodel_all_gt_8000_patch_{}.tfrecords'.format(cfg.other_class, 2048)
        path = os.path.join(cfg.inp_dir, filename)
    return path


Model3D = namedtuple('Model3D', 'id, name, voxels, mask, image, gt3d, camera, cam_pos, depth, num_views')
...
class Dataset3D:
    ...
    def load_data(self, cfg):
        ...
        for k, string_record in enumerate(record_iterator):
            ...
            float_list = example.features.feature['gt3d'].float_list.value 
  
            gt3d_1d = np.array(float_list)
            gt3ds = gt3d_1d.reshape((num_views, cfg.gt_size, 3))
            ...
            model = Model3D(id=k, name=model_name, voxels=voxels,
                            mask=masks, image=images, gt3d=gt3ds, camera=cameras, cam_pos=cam_pos,
