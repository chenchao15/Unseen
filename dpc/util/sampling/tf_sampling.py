...
if __name__=='__main__':
    ...
    testData = np.random.rand(1,1000,3)

    with tf.Session() as sess:
        idx = farthest_point_sample(10, testData)
        sampleData = gather_point(testData, idx)
        a ,b = sess.run([idx,sampleData])
        print(a.shape)
        print(b.shape)
        sample_idx = sess.run(idx)
    ...
    # sampleData = gather_point(testData, idx)
    # for id,data in zip(idx,sampleData):
    #     print('Index ',idx,' : ',data)
