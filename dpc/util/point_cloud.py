...
def pointcloud_project_fast(cfg, point_cloud, transform, predicted_translation,
    ...
    xy = translate_coord(cfg, xy)
    #coord = tf.reshape(xy, [num, cfg.vox_size, cfg.vox_size, 2])
    coord = xy    
    ...
    output = {
        "proj": tf.zeros([]),
        "voxels": tf.zeros([]),#voxels,
        "tr_pc": tf.zeros([]),#tr_pc,
        "voxels_rgb": tf.zeros([]),#voxels_rgb,
        "proj_rgb": tf.zeros([]),#proj_rgb,
        "drc_probs": tf.zeros([]),#drc_probs,
        "proj_depth": tf.zeros([]),#proj_depth,
        "coord": coord
    }
    return output
